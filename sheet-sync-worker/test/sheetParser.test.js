...
import { buildSheetRows, buildMergedSheetRows, cell } from './sheetParserFixtures.js';
...
test('fetchAndParseWeek: 依照 SHEET_MASTERS 清單逐一查詢分頁(泓文/哲瑋共用合併分頁)，結果合併起來、只留目標週範圍內的', async () => {
  const individualRows = buildSheetRows([{ dateSerials: FULL_WEEK_SERIALS, slots: { 0: [[1, '測試客人', null]] } }]);
  const mergedRows = buildMergedSheetRows([{ dateSerials: FULL_WEEK_SERIALS, slots: { 0: { 1: { 泓文: ['測試客人A', null], 哲瑋: ['測試客人B', null] } } } }]);
  ...
  const records = await fetchAndParseWeek(
    ...
    '2026-07-06',
    {
      ...
      fetchGridRows: async (env, { sheetTitle }) => {
        ...
        const rows = sheetTitle === '7月-合併' ? mergedRows : individualRows;
        return { title: sheetTitle, rows };
      },
    }
  );

  // 泓文/哲瑋共用同一次「7月-合併」查詢，不會各自查一次——3 個不同的
  // sheetTitle(合併、麒、治)，不是 4 個。
  assert.deepEqual(calledSheetTitles.sort(), ['7月-合併', '7月-治', '7月-麒'].sort());
  assert.equal(records.length, SHEET_MASTERS.length); // 泓文/哲瑋各一筆(合併分頁) + 麒/治各一筆(個別分頁) = 4 筆
});
...
test('fetchAndParseMonth: 依 SHEET_MASTERS 逐一查詢(泓文/哲瑋共用合併分頁)，且過濾掉跨月週區塊裡不屬於這個月的日期', async () => {
  ...
  const individualRows = buildSheetRows([
  ...
  const mergedRows = buildMergedSheetRows([
    { dateSerials: FULL_WEEK_SERIALS, slots: { 0: { 1: { 泓文: ['客人A', null] } } } },
    { dateSerials: augBoundarySerials, slots: { 0: { 0: { 泓文: ['客人B', null] }, 6: { 泓文: ['客人C', null] } } } },
  ]);
...
test('fetchAndParseMonth: 只呼叫 3 次(合併+麒+治)，泓文/哲瑋共用合併分頁不會各自查一次', async () => {
  const individualRows = buildSheetRows([{ dateSerials: FULL_WEEK_SERIALS, slots: {} }]);
  const mergedRows = buildMergedSheetRows([{ dateSerials: FULL_WEEK_SERIALS, slots: {} }]);
  const calledSheetTitles = [];
  ...
  assert.equal(calledSheetTitles.length, 3);
});
...
// ===== 合併分頁(parseMergedGridIntoRecords) =====

const HONGWEN = { name: '泓文', continuationMarks: ["''"] };
const ZHEWEI = { name: '哲瑋', continuationMarks: ["''"] };

test('合併分頁：泓文跟哲瑋各自只解析出自己的記錄，不會互相混到', async () => {
  const rows = buildMergedSheetRows([
    {
      dateSerials: FULL_WEEK_SERIALS,
      slots: { 0: { 3: { 泓文: ['王小明', null], 哲瑋: ['李小華', null] } } }, // weekday 3=週三=2026-07-08
    },
  ]);
  const hongwenRecords = await parseMergedGridIntoRecords(rows, HONGWEN);
  const zheweiRecords = await parseMergedGridIntoRecords(rows, ZHEWEI);

  assert.equal(hongwenRecords.length, 1);
  assert.equal(hongwenRecords[0].customerName, '王小明');
  assert.equal(hongwenRecords[0].masterName, '泓文');
  assert.equal(hongwenRecords[0].date, '2026-07-08');
  assert.equal(hongwenRecords[0].startTime, '08:00');

  assert.equal(zheweiRecords.length, 1);
  assert.equal(zheweiRecords[0].customerName, '李小華');
  assert.equal(zheweiRecords[0].masterName, '哲瑋');
  assert.equal(zheweiRecords[0].date, '2026-07-08');
});

test('合併分頁：某天的日期序號是 null(跨到別月)，這天兩位師傅的格子都要跳過', async () => {
  const serialsWithGap = [...FULL_WEEK_SERIALS];
  serialsWithGap[3] = null; // 週三這天不屬於這個月份分頁
  const rows = buildMergedSheetRows([
    { dateSerials: serialsWithGap, slots: { 0: { 3: { 泓文: ['王小明', null], 哲瑋: ['李小華', null] } } } },
  ]);
  const hongwenRecords = await parseMergedGridIntoRecords(rows, HONGWEN);
  assert.equal(hongwenRecords.length, 0);
});

test('合併分頁：黃底=新客、紅底=休假，跟個別分頁規則一致', async () => {
  const rows = buildMergedSheetRows([
    {
      dateSerials: FULL_WEEK_SERIALS,
      slots: {
  ...
  const records = await parseMergedGridIntoRecords(rows, HONGWEN);
  const newCustomer = records.find((r) => r.startTime === '08:00');
  const vacation = records.find((r) => r.startTime === '08:30');
  assert.equal(newCustomer.colorTag, 'new_customer');
  assert.equal(newCustomer.isNewCustomer, true);
  assert.equal(vacation.colorTag, 'vacation');
  assert.equal(vacation.customerName, '休假');
});

test('合併分頁：已知延續符號，同一位師傅連續格產生同行者記錄(不影響另一位師傅)', async () => {
  const rows = buildMergedSheetRows([
    {
      dateSerials: FULL_WEEK_SERIALS,
      slots: {
  ...
  const records = await parseMergedGridIntoRecords(rows, HONGWEN);
  assert.equal(records.length, 3);
  assert.equal(records[0].customerName, '張三');
  assert.equal(records[1].customerName, '張三-同行1');
  assert.equal(records[2].customerName, '張三-同行2');
});

test('合併分頁：#REF! 標記 needsReview，不會被當成正常姓名', async () => {
  const rows = buildMergedSheetRows([{ dateSerials: FULL_WEEK_SERIALS, slots: { 0: { 3: { 哲瑋: ['#REF!', null] } } } }]);
  const records = await parseMergedGridIntoRecords(rows, ZHEWEI);
  assert.equal(records[0].needsReview, true);
  assert.match(records[0].reviewReasons[0], /#REF!/);
});

test('合併分頁：多個週區塊都會被找到並處理', async () => {
  const rows = buildMergedSheetRows([
    { dateSerials: FULL_WEEK_SERIALS, slots: { 0: { 3: { 泓文: ['第一週', null] } } } },
    { dateSerials: FULL_WEEK_SERIALS.map((s) => s + 7), slots: { 0: { 3: { 泓文: ['第二週', null] } } } },
  ]);
  const records = await parseMergedGridIntoRecords(rows, HONGWEN);
  assert.equal(records.length, 2);
  assert.equal(records.find((r) => r.customerName === '第一週').date, '2026-07-08');
  assert.equal(records.find((r) => r.customerName === '第二週').date, '2026-07-15');
});

test('合併分頁：完全沒有「師傅」列時要丟清楚的錯誤', async () => {
  await assert.rejects(() => parseMergedGridIntoRecords([[cell(null)]], HONGWEN), /找不到任何「師傅」子表頭列/);
});

test('合併分頁：資料列後面接的不是「每日人數」時要丟清楚的錯誤(結構跟預期不符)', async () => {
  const rows = buildMergedSheetRows([{ dateSerials: FULL_WEEK_SERIALS, slots: {} }]);
  // 故意把「每日人數」列改掉，模擬結構跟預期不符的情況
  const totalRowIdx = rows.findIndex((r) => r?.[0]?.value === '每日人數');
  rows[totalRowIdx][0] = cell('不是每日人數');
  await assert.rejects(() => parseMergedGridIntoRecords(rows, HONGWEN), /應該是「每日人數」小計列/);
});

test('合併分頁：師傅子表頭列裡找不到指定的師傅名字時要丟清楚的錯誤', async () => {
  const rows = buildMergedSheetRows([{ dateSerials: FULL_WEEK_SERIALS, slots: {} }]);
  const fakeMaster = { name: '不存在的師傅', continuationMarks: [] };
  await assert.rejects(() => parseMergedGridIntoRecords(rows, fakeMaster), /找不到「不存在的師傅」/);
});

test('合併分頁：泓文跟哲瑋在同一個時段各自的 identityKey 不同，不會互相覆蓋', async () => {
  const rows = buildMergedSheetRows([
    { dateSerials: FULL_WEEK_SERIALS, slots: { 0: { 3: { 泓文: ['甲', null], 哲瑋: ['乙', null] } } } },
  ]);
  const hongwenRecords = await parseMergedGridIntoRecords(rows, HONGWEN);
  const zheweiRecords = await parseMergedGridIntoRecords(rows, ZHEWEI);
  assert.notEqual(hongwenRecords[0].identityKey, zheweiRecords[0].identityKey);
});

// ─────────────────────────────────────────
// 「(脆)」文字標記(2026-07-13)：Hanna在Sheet上手動打在客人名字後面的
// 文字慣例，匯入時要偵測、從名字裡拿掉、轉成colorTag='cui'，不管
// 儲存格底色是什麼(文字標記優先權蓋過底色判斷)。
// ─────────────────────────────────────────
test('(脆)標記：全形括號會被偵測、從名字拿掉、colorTag變成cui', async () => {
  const rows = buildSheetRows([
    { dateSerials: FULL_WEEK_SERIALS, slots: { 0: [[3, '蔡孟奇（脆）', null]] } },
  ]);
  const master = { name: '麒', continuationMarks: ['\u201D'] };
  const records = await parseGridIntoRecords(rows, master);
  assert.equal(records.length, 1);
  assert.equal(records[0].customerName, '蔡孟奇', '（脆）文字要從名字裡完全拿掉');
  assert.equal(records[0].colorTag, 'cui');
});

test('(脆)標記：半形括號也要認得', async () => {
  const rows = buildSheetRows([
    { dateSerials: FULL_WEEK_SERIALS, slots: { 0: [[3, '陳小華(脆)', null]] } },
  ]);
  const master = { name: '麒', continuationMarks: ['\u201D'] };
  const records = await parseGridIntoRecords(rows, master);
  assert.equal(records[0].customerName, '陳小華');
...
test('(脆)標記：文字標記優先權蓋過儲存格底色(黃色底色本來會判成new_customer，但有(脆)文字就以脆為準)', async () => {
  const rows = buildSheetRows([
    { dateSerials: FULL_WEEK_SERIALS, slots: { 0: [[3, '王小明（脆）', '#FFFF00']] } },
  ]);
  const master = { name: '麒', continuationMarks: ['\u201D'] };
  const records = await parseGridIntoRecords(rows, master);
  assert.equal(records[0].colorTag, 'cui', '文字標記要贏過底色判斷');
  assert.equal(records[0].isNewCustomer, false, 'cui不是new_customer，isNewCustomer不該被誤觸發');
});

test('(脆)標記：沒有這個文字的一般預約完全不受影響(回歸測試)', async () => {
  const rows = buildSheetRows([
    { dateSerials: FULL_WEEK_SERIALS, slots: { 0: [[3, '林先生', '#FFFF00']] } },
  ]);
  const master = { name: '麒', continuationMarks: ['\u201D'] };
  const records = await parseGridIntoRecords(rows, master);
  assert.equal(records[0].customerName, '林先生');
  assert.equal(records[0].colorTag, 'new_customer', '沒有(脆)文字時，底色判斷邏輯要維持原樣');
});

test('(脆)標記：延續格自己也打了(脆)是不合理的極端情況——延續格代表「跟本人同一筆」，\n  本來就是沿用anchor(本人)的colorTag，不是看這一格自己的文字。驗證：(1)拿掉(脆)後剩下\n  的符號仍然能正確比對成延續格(不會因為多打了(脆)兩個字就比對失敗、被誤判成全新的人)，\n  (2)colorTag沿用anchor(這裡anchor本人沒有標記，維持既有的「同行者沿用本人顏色」規則，\n  不會被這一格自己的(脆)文字覆蓋掉)', async () => {
  const rows = buildSheetRows([
    { dateSerials: FULL_WEEK_SERIALS, slots: { 0: [[3, '王小明', null]], 1: [[3, '\u201D（脆）', null]] } },
  ]);
  const master = { name: '麒', continuationMarks: ['\u201D'] };
  const records = await parseGridIntoRecords(rows, master);
  assert.equal(records.length, 2, '拿掉(脆)後的乾淨文字要能正確比對成延續格，不會因為多打了(脆)而被誤判成全新客人');
  assert.equal(records[1].colorTag, records[0].colorTag, '同行者沿用本人(anchor)的colorTag是既有規則，不受這一格自己的(脆)文字影響');
});
